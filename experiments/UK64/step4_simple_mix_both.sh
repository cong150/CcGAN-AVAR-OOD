...
###############################################################################
# 步骤4-4：Simple-Mix + L_perturb + L_interp（两个正则化都用）
#
# 说明：
# - 使用混合数据集：ID区域全部 + OOD区域少量
# - 同时使用L_perturb和L_interp正则化
###############################################################################
...
echo "╔══════════════════════════════════════════════════════════╗"
echo "║  步骤4-4：Simple-Mix + Both (L_perturb+L_interp)          ║"
echo "╚══════════════════════════════════════════════════════════╝"
echo ""

# ==================== 实验参数 ====================
DATA_NAME="UTKFace"
SETTING="simple_mix_both"
VISDOM_ENV="${SETTING}"
SEED=2025
...
# 使用混合数据集
TRAIN_DATA="experiments/data/UTKFace_mixed_id_full_ood_5_64x64.h5"
...
python main.py \
    ...
    --root_path "${ROOT_PATH}" \
    --data_path "${DATA_PATH}" \
    --train_data_h5_path "${TRAIN_DATA}" \
    --seed "${SEED}" \
    --min_label "${MIN_LABEL}" \
    --max_label "${MAX_LABEL}" \
    --img_size "${IMG_SIZE}" \
    --max_num_img_per_label -1 \
    --net_name "${NET_NAME}" \
    --dim_z "${DIM_GAN}" \
    --dim_y "${DIM_Y}" \
    --gene_ch "${GENE_CH}" \
