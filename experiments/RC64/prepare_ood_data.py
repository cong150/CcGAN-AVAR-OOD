...
def prepare_ood_split(data_path, output_path, train_min, train_max, test_min, test_max, 
    ...
    Args:
        data_path: 原始RC-49数据集路径
        output_path: 输出路径
        train_min: 训练集最小角度
        train_max: 训练集最大角度
        test_min: 测试集最小角度
        test_max: 测试集最大角度
        ood_samples_per_label: OOD区域每个角度保留的样本数（None表示全部）
        seed: 随机种子
    """
    np.random.seed(seed)
    ...
    print(f"\n{'='*80}")
    print(f"准备OOD数据划分:")
    print(f"  训练区域 (ID): [{train_min}, {train_max}]")
    print(f"  测试区域 (OOD): [{test_min}, {test_max}]")
    if ood_samples_per_label:
        print(f"  OOD区域每个标签样本数: {ood_samples_per_label}")
    print(f"{'='*80}\n")
    ...
    # 统计信息
    print("标签分布统计:")
    print(f"  ID区域 - 唯一标签数: {len(np.unique(id_labels))}")
    print(f"  OOD区域 - 唯一标签数: {len(np.unique(ood_labels))}")
...
def visualize_data_split(output_path, save_fig_path=None):
    ...
    ax1.set_title(f'ID Region [{train_min}, {train_max}] - Training Data')
...
def create_mixed_training_set(data_path, output_path, id_min, id_max, ood_min, ood_max, 
    ...
    Args:
        ...
        ood_samples_per_label: OOD区域每个标签的样本数
    """
    np.random.seed(seed)
    
    print(f"\n{'='*80}")
    print("创建混合训练集:")
    print(f"  ID区域 (全部数据): [{id_min}, {id_max}]")
    print(f"  OOD区域 (少量数据): [{ood_min}, {ood_max}], 每标签{ood_samples_per_label}张")
    print(f"{'='*80}\n")
    ...
    print(f"✓ ID区域 [{id_min}, {id_max}]: {len(id_images)} 张（全部）")
    ...
    print(f"✓ OOD区域 [{ood_min}, {ood_max}]: {len(ood_images)} 张（采样后）")
...
if __name__ == "__main__":
    ...
    # For split mode
    parser.add_argument('--train_min', type=float, default=0.0)
    parser.add_argument('--train_max', type=float, default=45.0)
    parser.add_argument('--test_min', type=float, default=45.0)
    parser.add_argument('--test_max', type=float, default=90.0)
    ...
    parser.add_argument('--ood_samples_per_label', type=int, default=50,
                       help='OOD区域每个标签的样本数限制')
